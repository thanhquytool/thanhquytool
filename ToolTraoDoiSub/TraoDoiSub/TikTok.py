...
\033[1;37m══════════════════════════════════════════════════════════════
\033[1;97m[\033[1;91m📝\033[1;97m]\033[1;97m Tool\033[1;31m     : \033[1;31m☞ TraoDoiSub.com - Tik Tok🧸 ☜
...
def main():
	...
	banner()
	while True:
		if os.path.exists('token_traodoisub.txt'):
			with open('token_traodoisub.txt', 'r') as f:
				token = f.read()
			...
			try:
				...
				if chon == '2':
					os.remove('token_traodoisub.txt')
				elif chon == '1':
					pass
				else:
				...
				os.remove('token_traodoisub.txt')
		if not os.path.exists('token_traodoisub.txt'):
			token = input(f'{huong_pc}{luc}Nhập Access_Token TDS: {vang}')
			with open('token_traodoisub.txt', 'w') as f:
				f.write(token)
		with open('token_traodoisub.txt', 'r') as f:
			token = f.read()
		tds = TraoDoiSub_Api(token)
